...
const FeaturedSection = () => {
  ...
  const handleFoodsUpdate = useCallback((action: string, data: any) => {
    setFoods((prev) => {
      if (action === 'created') {
        if (data.isFeatured) return [...prev, data];
        return prev;
  ...
  return (
    <section className="py-24 px-6 relative">
      <div className="max-w-7xl mx-auto">
        <div className="mb-16">
          <h2 className="text-5xl font-bold mb-4">Featured <span className="text-primary-gold">Dishes</span></h2>
          <p className="text-gray-400 text-lg max-w-2xl">Discover our best-selling and most-loved dishes crafted to perfection</p>
        </div>
        ...
        <div className="text-center mt-16">
          <a href="/menu">
            <button className="btn btn-primary px-8 py-4 text-lg">Explore Full Menu →</button>
          </a>
        </div>
      </div>
    </section>
