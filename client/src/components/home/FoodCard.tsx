...
const FoodCard = ({ id, name, price, image, rating, isVegetarian, foodType, categoryName, description }: FoodCardProps) => {
  ...
  return (
    <>
      <div className="card h-full overflow-hidden group hover:-translate-y-1 hover:shadow-[0_20px_40px_rgba(212,175,55,0.15)] transition-all duration-300">
        <div className="relative w-full h-48 overflow-hidden rounded-xl mb-4">
          {image && !image.startsWith('http') ? (
            ...
            <img src={image || '/placeholder.jpg'} alt={name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" loading="lazy" />
          )}
          ...
          <button onClick={handleWishlist} className="absolute top-3 left-3 p-2 rounded-full bg-dark-bg/80 hover:bg-dark-bg transition-colors" title={isInWishlist(id) ? 'Remove from wishlist' : 'Add to wishlist'}>
        ...
        <div className="space-y-3 flex-grow">
          <div>
            <h3 className="text-lg font-bold text-white group-hover:text-primary-gold transition-colors">{name}</h3>
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`px-3 py-2 rounded-lg font-semibold transition-all text-sm ${
                  isInCart(id) ? "bg-green-500/20 text-green-400 hover:bg-green-500/30" : "bg-primary-gold/20 text-primary-gold hover:bg-primary-gold/30"
                }`}
              >
                {isInCart(id) ? "✓ In Cart" : "🛒 Add"}
              </button>
              <Link to={`/food/${id}`}>
                <button className="px-3 py-2 bg-primary-gold text-dark-bg rounded-lg font-semibold hover:bg-accent-gold transition-all text-sm">View</button>
              </Link>
            </div>
          </div>
        </div>
      ...
      <LoginDialog open={showLoginDialog} onClose={() => setShowLoginDialog(false)} onLoginSuccess={() => setShowAddToCartModal(true)} />
      <AddToCartModal open={showAddToCartModal} onClose={() => setShowAddToCartModal(false)} foodId={id} name={name} initialPrice={price} />
    </>
  );
};
