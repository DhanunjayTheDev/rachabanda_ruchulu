...
export default function GlobalRealtimeProvider() {
  ...
  const handleFoods = useCallback(
    (action: string, data: any) => {
      if (action === 'created') {
        addNotification({ type: 'food', title: 'New Dish Added!', message: `${data.name} is now available on the menu.`, data });
      } else if (action === 'updated') {
        addNotification({ type: 'food', title: 'Menu Updated', message: `${data.name} has been updated.`, data });
      } else if (action === 'deleted') {
        addNotification({ type: 'food', title: 'Dish Removed', message: 'A dish has been removed from the menu.', data });
      }
    },
    [addNotification]
  );
  ...
  const handleCategories = useCallback(
    (action: string, data: any) => {
      if (action === 'created') {
        addNotification({ type: 'category', title: 'New Category', message: `Category "${data.name}" has been added.`, data });
      } else if (action === 'updated') {
        addNotification({ type: 'category', title: 'Category Updated', message: `Category "${data.name}" has been updated.`, data });
      }
    },
    [addNotification]
  );
  ...
  const handleCoupons = useCallback(
    (action: string, data: any) => {
      if (action === 'created') {
        ...
        addNotification({ type: 'coupon', title: 'Coupon Updated', message: `Coupon ${data.code} has been updated.`, data });
      } else if (action === 'deleted') {
        addNotification({ type: 'coupon', title: 'Coupon Expired', message: 'A coupon has been removed.', data });
      }
    },
    [addNotification]
  );
  ...
  const handleAnnouncements = useCallback(
    (action: string, data: any) => {
      if (action === 'created') {
        addNotification({ type: 'announcement', title: `📢 ${data.title}`, message: data.description || 'New announcement from the restaurant.', data });
      } else if (action === 'updated') {
        addNotification({ type: 'announcement', title: 'Announcement Updated', message: data.title, data });
      }
    },
    [addNotification]
  );
