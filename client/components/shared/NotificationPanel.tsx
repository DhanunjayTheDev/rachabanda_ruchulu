...
// Toast pop-up for a single new notification
function NotificationToast({
  ...
  return (
    ...
    >
      ...
      <button className="text-gray-400 hover:text-white shrink-0" onClick={onDismiss}>
        ✕
      </button>
    </motion.div>
  );
...
// Full notification panel (slide-in from bottom-right)
        ...
        <div className="flex items-center gap-2">
          {notifications.some((n) => !n.read) && (
            <button
              ...
              className="text-xs text-primary-gold hover:text-yellow-300 transition-colors"
            >
              Mark all read
            </button>
          )}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {!notif.read && (
                    <span className="shrink-0 w-2 h-2 bg-primary-gold rounded-full" />
                  )}
                </div>
...
// The main exported component - renders both the bell button and all panels
export default function NotificationPanel() {
  ...
  useEffect(() => {
    ...
    if (unread.length > prevCountRef.current) {
      ...
      // Browser push notification ONLY for orders, if tab is not focused
  ...
  // Don't render bell button on server to avoid hydration mismatch
  if (!mounted) {
    return null;
  }
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      ...
      {/* Bell button and drawer - only show when logged in */}
      {isLoggedIn() && (
...
// Hook for requesting browser notification permission on first load
